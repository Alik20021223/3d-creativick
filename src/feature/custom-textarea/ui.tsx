...
export function CustomTextArea<T extends FieldValues>({
  ...
  // следим за значением поля на верхнем уровне (чтобы можно было использовать хуки)
  const currentValue = useWatch({ control: ctl, name }) as unknown as string | undefined;
          ...
          <div className={cn('space-y-1', containerClassName)}>
            ...
            <div className='relative'>
              <Textarea
                ...
                className={cn((clearable || showCounter) && 'pr-10', className)}
              />
              ...
              {clearable && (
                <button
                  ...
                  className='text-muted-foreground absolute top-2 right-2 flex h-6 w-6 items-center justify-center rounded-md hover:bg-black/5'
                >
              ...
              {showCounter && (
                <div className='text-muted-foreground pointer-events-none absolute right-2 bottom-2 text-xs select-none'>
                  {typeof maxLength === 'number' ? `${len}/${maxLength}` : len}
                </div>
              )}
            </div>
