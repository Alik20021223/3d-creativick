...
const CustomDropdown: React.FC<CustomDropdownProps> = ({
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger className='rounded-[12px] bg-white px-3.5 py-2' asChild>
        {trigger ? (
        ...
        {items.map((it) => {
          ...
          return (
            <DropdownMenuItem
              ...
              className={cn(
                'relative flex cursor-pointer items-center gap-2 rounded-[12px] px-2 py-1.5 text-sm select-none',
                // базовый цвет
                !isSelected && 'text-secondary-text',
                // выбранный пункт — меняем фон и текст
                isSelected && 'text-primary bg-secondary-white font-semibold',
                it.destructive &&
            ...
            >
              ...
              {it.shortcut && <span className='text-muted-foreground text-xs'>{it.shortcut}</span>}
            </DropdownMenuItem>
          );
        })}
      </DropdownMenuContent>
