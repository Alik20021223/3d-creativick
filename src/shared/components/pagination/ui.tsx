...
export default function PaginationCustom({
  ...
  const activePage = 'bg-white text-primary shadow scale-115';
  const inactivePage = 'bg-white/10 text-white hover:bg-white/20';
      ...
      <PaginationContent className='gap-2'>
        <PaginationItem>
          <PaginationPrevious
            ...
            className={cn(
              'rounded-full border border-white px-3 py-2 text-white',
          ...
          it === '…' ? (
            ...
            <PaginationItem key={it}>
              <PaginationLink
                ...
                className={cn(basePage, it === clampedPage ? activePage : inactivePage)}
                onClick={(e) => {
                  e.preventDefault();
