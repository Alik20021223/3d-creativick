...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const isPassword = type === "password" && typeof show === "boolean" && typeof setShow === "function"

    return (
      <div className="relative w-full">
        {leftIcon && (
          <div className="absolute inset-y-0 left-2 flex items-center text-muted-foreground pointer-events-none">
            {leftIcon}
          </div>
        )}
        <input
        ...
        {rightIcon &&
          <div className="absolute inset-y-0 right-2 flex items-center text-muted-foreground pointer-events-none">
            {rightIcon}
          </div>}
      </div>
    )
